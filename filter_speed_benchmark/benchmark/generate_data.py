...
class SampleGenerator:
    ...
    def generate(self, path, num_queries, payload_keys=None):
        ...
        with open(path + f'.queries.jsonl', 'w') as out:
            for _ in tqdm(range(num_queries)):
                out.write(json.dumps(self.generate_query({})))
                out.write('\n')

        if payload_keys:
            for payload_key in payload_keys:
                with open(path + f'.queries-{payload_key}.jsonl', 'w') as out:
                    for _ in tqdm(range(num_queries)):
...
if __name__ == '__main__':
    ...
    parser.add_argument("-p", "--parts", action='append', type=int, help="payload fractions")
    ...
    dim = args.dim
    payload_params = dict(zip(string.ascii_lowercase, args.parts or []))
